...
def create_default_template(save_path):
    ...
    font_small = Font(name='맑은 고딕', size=9, color='616161')  # 설명용 작은 글씨

    align_c = Alignment(horizontal='center', vertical='center')
    align_r = Alignment(horizontal='right', vertical='center')
    align_l_top = Alignment(horizontal='left', vertical='top', wrap_text=True)  # 줄바꿈 필수
    ...
    data = [
        ...
        # [수정] 산출 근거 (높이 확장 및 내용 통합)
        (19, 1, "상세 산출 내역 및 법적 기준", "SEC_TITLE"),
        # 설명이 길어지므로 4줄을 병합하고 높이를 키움 (20~23행)
        (20, 1, "{{calc_detail}}", "BOX_DETAIL"),
            ...
            if cell.value is not None:
                ...
                for key, val in data_context.items():
                    ...
                    if re.search(pattern, text):
                        # 줄바꿈 문자 처리를 위해 문자열로 변환
                        val_str = str(val)
                        if re.fullmatch(pattern, text.strip()):
                            cell.value = val  # 숫자형 유지 등을 위해 원본 대입 시도
                            if isinstance(val, str):
                                cell.value = val  # 문자열이면 그대로
                        else:
                            cell.value = re.sub(pattern, val_str, text)
                        replaced_count += 1
                        text = str(cell.value)
    try:
        wb.save(save_path)
